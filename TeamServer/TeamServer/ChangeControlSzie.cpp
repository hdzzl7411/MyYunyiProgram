...
CChangeControlSzie::CChangeControlSzie(UINT nID, CWnd* pParent)
	: CDialog(nID, pParent)
	, m_iClientWidth(0)
	, m_iClientHeight(0)
	, m_iMinWidth(0)
...
BEGIN_MESSAGE_MAP(CChangeControlSzie, CDialog)
	ON_WM_SIZE()
	ON_WM_SIZING()
END_MESSAGE_MAP()

BOOL CChangeControlSzie::OnInitDialog()
{
...
void CChangeControlSzie::OnSize(UINT nType, int cx, int cy)
{
...
void CChangeControlSzie::OnSizing(UINT nSide, LPRECT lpRect)
{
...
BOOL CChangeControlSzie::SetControlProperty(PDLGCTLINFO lp, int nElements)
{
	//  设置控件数组信息
...
void CChangeControlSzie::ShowSizeIcon(BOOL bShow )
{
