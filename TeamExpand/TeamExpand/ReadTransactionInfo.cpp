...
void CReadTransactionInfo::AddEvaluationToList ( CSEvaluation csEva )
{
	...
	{
		if ( EvaIt->nCounterIndex == csEva.nCounterIndex )
		{
			EvaIt->nCount++;
			if ( csEva.nEvaType == 1 )
			{
...
void CReadTransactionInfo::AddRunInfoToList ( CSRuningInfo csRun )
{
	...
	{
		if ( RunIt->nCounterIndex == csRun.nCounterIndex )
		{
			if ( csRun.nTransactionStatus == 1 )         //处于呼叫办理状态，更新票号
			{
				memcpy(RunIt->nCounterIndex , csRun.nCounterIndex,3);
				memcpy(RunIt->nTicketNumber, csRun.nTicketNumber, 6);
			}
			bExist = TRUE;
		}
	}
